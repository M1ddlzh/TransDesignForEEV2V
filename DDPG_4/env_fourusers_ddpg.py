...
class EnvFourUsers(gym.Env):
    ...
    def __init__(self, steps_in_one_episode):
        ...
        self.scale_ij_3 = np.sqrt(0.0003 / 2)
        ...
        self.action_min = np.array([self.a_min]*4, dtype=np.float32)
        self.action_max = np.array([self.a_max]*4, dtype=np.float32)
    ...
    def get_channel_h(self):
        ...
        h44 = (self.np_random.rayleigh(self.scale_ii)) ** 2
        ...
        h14 = (self.np_random.rayleigh(self.scale_ij_3)) ** 2
        h41 = (self.np_random.rayleigh(self.scale_ij_3)) ** 2
        h24 = (self.np_random.rayleigh(self.scale_ij_2)) ** 2
        h42 = (self.np_random.rayleigh(self.scale_ij_2)) ** 2
        h34 = (self.np_random.rayleigh(self.scale_ij)) ** 2
        h43 = (self.np_random.rayleigh(self.scale_ij)) ** 2
        H = [h11, h22, h33, h44, h12, h21, h13, h31, h14, h41, h32, h23,
            h42, h24, h34, h43]
        return H
    ...
    def step(self, action):
        ...
        self.ty2_gene_rate_4 = self.np_random.poisson(self.ty2_lamda)

        h11, h22, h33, h44, h12, h21, h13, h31, h14, h41, h32, h23, \
            h42, h24, h34, h43 = self.H
        self.I1 = self.band_width * np.log2(
            1 + h11 * p1_w / (self.N0 + h12 * p2_w + h13 * p3_w + h14 * p4_w))     
        self.I2 = self.band_width * np.log2(
            1 + h22 * p2_w / (self.N0 + h21 * p1_w + h23 * p3_w + h24 * p4_w)) 
        self.I3 = self.band_width * np.log2(
            1 + h33 * p3_w / (self.N0 + h31 * p1_w + h32 * p2_w + h34 * p4_w))
        self.I4 = self.band_width * np.log2(
            1 + h44 * p4_w / (self.N0 + h41 * p1_w + h42 * p2_w + h43 * p3_w))
        ...
        reward_4, self.ty1_sum_succ_4, self.Q_len_4, self.sum_r_4, \
            self.sum_p_4, self.energy_effi_4 \
            = self.get_feedback(
                p4_w, 0, self.I4, self.ty2_gene_rate_4, 
                self.ty1_sum_succ_4, self.Q_len_4, self.sum_r_4, self.sum_p_4)
        ...
        if self.Q_len_4 > self.Q_max:
            self.large_than_Q_4 += 1
        ...
        next_state = np.array([
            ...
            # self.ty1_sum_succ_4 / self.step_in_one_episode * self.s_amp, 
            self.Q_len_1 / self.Q_shr, self.Q_len_2 / self.Q_shr, 
            self.Q_len_3 / self.Q_shr, self.Q_len_4 / self.Q_shr, 
            h11 * self.h_amp_self, h22 * self.h_amp_self, 
            h33 * self.h_amp_self, h44 * self.h_amp_self, 
            h12 * self.h_amp_betw, h21 * self.h_amp_betw,
            h13 * self.h_amp_betw, h31 * self.h_amp_betw,
            h14 * self.h_amp_betw, h41 * self.h_amp_betw,
            h32 * self.h_amp_betw, h23 * self.h_amp_betw,
            h42 * self.h_amp_betw, h24 * self.h_amp_betw,
            h34 * self.h_amp_betw, h43 * self.h_amp_betw,
            # self.step_now / self.step_in_one_episode * 10
            ])

        reward = (reward_1 + reward_2 + reward_3 + reward_4) / 4
        ...
        if self.step_now == self.step_in_one_episode:
            ...
            info = {
                ...
                'ty1_succ_rate_4': self.ty1_sum_succ_4 / self.step_in_one_episode,
                'Q_len_1': self.Q_len_1,
                'Q_len_2': self.Q_len_2,
                'Q_len_3': self.Q_len_3,
                'Q_len_4': self.Q_len_4,
                'energy_effi_1': self.energy_effi_1,
                'energy_effi_2': self.energy_effi_2, 
                'energy_effi_3': self.energy_effi_3, 
                'energy_effi_4': self.energy_effi_4, 
                'avg_rate': ((self.sum_r_1 + self.sum_r_2 + self.sum_r_3 + self.sum_r_4)
                    / self.step_in_one_episode / 4),
                'avg_power': ((self.sum_p_1 + self.sum_p_2 + self.sum_p_3 + self.sum_p_4) 
                    / self.step_in_one_episode / 4),
                }
...
if __name__ == '__main__':
    # testing
    env = EnvFourUsers(500)
    env.seed(0)
    print(env.reset())

    power = [[9, 9, 0, 4], [5, 5, 0, 4], [0, 0, 0, 4], [-5, -5, 0, 4], [-9, -9, 0, 4]]
    for p in power:
        print(env.step(p))
