...
from env_twousers_ddpg import EnvTwoUsers
...
def offpolicy_trainer(
    ...
    for epoch in range(1, 1 + max_epoch):
        ...
        # test
        # change
        if epoch % 50 == 0: # or epoch < 20000:
            env = EnvTwoUsers(step_per_epoch)
            # env.seed(0)
            policy.train(False)
            ...
            if verbose:
                ...
                print(f'Epoch #{epoch}: test_reward: {result["rew"]:.6f}, '
                    ...
                    f'avg_power: {result["avg_p"]/ep:.4f} \n')
            # change
            training_res.append([(result["ee_1"][0]/ep + result["ee_2"][0]/ep) / 2, 
                (result["ty1s_1"][0]/ep + result["ty1s_2"][0]/ep) / 2,
                (result["ql_1"][0]/ep + result["ql_2"][0]/ep) / 2,
                result["rew"]])
    ...
    wb.save("training_directly_slot" + str(step_per_epoch) + ".xlsx")
    test_collector.collect_time = -1    # we do not use this function, set "-1" for simply.
    return gather_info(
