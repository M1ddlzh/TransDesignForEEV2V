...
def get_args():
    ...
    parser.add_argument('--task', type=str, default='threeusers')
    ...
    parser.add_argument('--unit_num', type=int, default=64)
...
def training_ddpg(args=get_args()):
    ...
    train_envs = VectorEnv(
        [lambda: EnvThreeUsers(args.step_per_epoch) for _ in range(args.training_num)])
    test_envs = VectorEnv(
        [lambda: EnvThreeUsers(args.step_per_epoch) for _ in range(args.test_num)])
    # seed
...
def testing_ddpg(args=get_args()):
    ...
    print('large than Qmax: users1: {}, users2: {}, users3: {}.'
